...
public class DemoScript : MonoBehaviour {
	...
	public Texture2D imageToReset;
	public GameObject tesseractObj; 
	...
	public void OnResetScreenshotPress()
	{
		screenshot.sprite = Sprite.Create(imageToReset, new Rect(0, 0, imageToReset.width, imageToReset.height), new Vector2(.5f, .5f));
		screenshot.color = Color.white;
		ui.alpha = 1;
		//
		tesseractObj.GetComponent<TesseractDemoScript>().CaptureImageActive = false;
		//
		console.text = "OCR Output:";
	}
	...
	void ScreenshotTaken(Texture2D image)
	{
		...
		ui.alpha = 1;
		//
		tesseractObj.GetComponent<TesseractDemoScript>().SetCaptureImageOCR(image);
		//
	}
